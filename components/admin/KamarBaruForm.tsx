...
export default function KamarBaruForm({ propertyId }: { propertyId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/rooms', {
      ...
      body: JSON.stringify({
        ...
        price_weekday: parseFloat(form.price_weekday) || 0,
        price_weekend: parseFloat(form.price_weekend) || 0,
      }),
    })
    ...
    <form onSubmit={handleSubmit} style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <Field label="Nama Paket *">
          <input
            type="text" required value={form.room_type}
            onChange={(e) => update('room_type', e.target.value)}
            placeholder="Penuh (2 Kamar Tidur)"
            style={inputStyle}
          />
        </Field>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          type="submit" disabled={loading || !form.room_number}
          style={{
            padding: '7px 16px', borderRadius: 'var(--radius)',
            border: 'none', background: 'var(--accent)', color: '#fff',
            fontSize: 13, fontWeight: 500,
            cursor: (loading || !form.room_number) ? 'not-allowed' : 'pointer',
            opacity: (loading || !form.room_number) ? 0.5 : 1,
            fontFamily: 'var(--font-sans)',
          }}
        >
